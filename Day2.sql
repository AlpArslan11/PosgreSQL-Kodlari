...
insert into personel values(100,'Hasan','İstanbul',5000,'ABC','Üsküdar');
...
insert into personel_adres values (101,'Kastamonu','Merkez');

select * from personel

select * from personel_adres

-- DML --> Data Manupulation Lang. (veri işleme dili)
...
--TCL - Transaction Control Lang. (işlem kontrol dili)
...
-- Personel tablosuna cinsiyet Varchar(20) ve yas int şeklinde yeni sütunlar ekleyiniz.
alter table personel add cinsiyet varchar(20), add yas int;

-- Personle tablosundan sirket field'ini siliniz.
alter table personel drop column sirket;

-- Personel tablosundaki sehir sutununun adını ülke olarak değiştirelim
...
-- Primary Key --> Bir sütunun null içermemesini ve sütundaki verilerin BENZERSİZ olmasını sağlar.(NOT NULL - UNIQ)
-- Foregin Key --> Başka bir tablodaki PRIMARY KEY'i referans göstermek için kullanılır.
...
isim VARCHAR(50) UNIQUE, -- UNIQUE --> Bir sütundaki tüm değerlerin BENZERSİZ yani tek olmasını sağlar.
maas int NOT NULL, -- NOT NULL --> Bir sütunun NULL içermemesini yani boş olmamasını sağlar.
ise_baslama DATE
);
