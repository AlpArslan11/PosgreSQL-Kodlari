-- GROUP BY --
/*
GROUP BY cümleciği bir SELECT ifadesinde satırları, sutunları değerlerine göre özet olarak gruplamak için kullanılır
GROUP BY cümleciği her grup başına satır döndürür.
GROUP BY genelde, AVG(), COUNT(), MAX() ve SUM() gibi aggregate fonksiyonları ile birlikte kullanılır.
*/
...
select isim, sum(urun_miktari) from manav group by isim
			-- sum(urun_miktari)as toplam_urun from manav group by isim // dersek stunun adını toplam_urun yapar
			-- ilkinde sutun adını sum yaptı
...
select urun_adi, min(urun_miktari) as minimun_urun_mik, max(urun_miktari) as maksimum_urun_mik from manav
where urun_adi is not null
group by urun_adi
...
-- personelin çalıştığı ülkeleri listeleyiniz
...
select ulke, cinsiyet, count(*)    -- * deyince ülke ve cinsiyet sutunundaki herşeyi saydıracak
as calisan_sayisi, round(avg(yas)) as yas_ortalamasi 
from personel
...
select ulke, cinsiyet, round(avg(yas)) as ortalama_yas, count(*) as calisan_sayisi
from personel
where maas>30000
